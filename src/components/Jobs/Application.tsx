...
export interface CrewApplicationData {
  ...
  seamansBooks?: Array<
    SeamansBookItem & { _id?: string; uploadStatus?: string }
  >;
  visas?: Array<VisaItem & { _id?: string; uploadStatus?: string }>;
  endorsements?: Array<
    EndorsementItem & { _id?: string; uploadStatus?: string }
  >;
  stcwCertificates?: Array<
    CertificateItem & { _id?: string; uploadStatus?: string }
  >;
...
const emptyLicence = (): LicenceItem => ({
  licenceType: "coc",
  country: "",
  grade: "",
  number: "",
...
const emptyPassport = (): PassportItem => ({
  number: "",
  country: "",
  placeIssued: "",
  dateIssued: "",
...
const emptySeaman = (): SeamansBookItem => ({
  number: "",
  country: "",
  placeIssued: "",
  dateIssued: "",
...
const emptyEndorse = (): EndorsementItem => ({
  name: "",
  country: "",
  number: "",
  placeIssued: "",
...
const emptyCert = (): CertificateItem => ({
  name: "",
  courseNumber: "",
  placeIssued: "",
  dateIssued: "",
...
const emptySeaExp = (): SeaExperienceItem => ({
  vesselName: "",
  flag: "",
  grt: "",
  vesselType: "",
...
const STEPS = [
  {
    ...
    description: "Identity, contact and physical details",
  },
  {
    id: 2,
    title: "Certificate of Competency",
    description: "CoC licences issued by flag states",
  },
  {
    id: 3,
    title: "Certificate of Equivalent",
    description: "CoE licences issued by flag states",
  },
  { id: 4, title: "Passport", description: "All passports, including expired" },
  {
    id: 5,
    title: "Seaman's Book",
    description: "All seaman's books, including expired",
  },
  { id: 6, title: "Visa", description: "" },
  {
    id: 7,
    title: "Endorsements",
    description: "Flag-state endorsements and attestations",
  },
...
function useArray<T>(initial: T[]) {
  ...
  const add = useCallback(
    (empty: T) => setItems((prev) => [...prev, empty]),
    [],
  );
...
const COVERALL_OPTIONS = ["XS", "S", "M", "L", "XL", "XXL", "XXXL"].map(
  (s) => ({ value: s, label: s }),
);
...
function sanitizeLicence(
  l: LicenceItem & { _id?: string; uploadStatus?: string },
): LicenceItem {
  return {
...
function sanitizePassport(
  p: PassportItem & { _id?: string; uploadStatus?: string },
): PassportItem {
  return {
    number: p.number ?? "",
    country: p.country ?? "",
...
function sanitizeSeaman(
  s: SeamansBookItem & { _id?: string; uploadStatus?: string },
): SeamansBookItem {
  return {
    number: s.number ?? "",
    country: s.country ?? "",
    placeIssued: s.placeIssued ?? "",
    dateIssued: s.dateIssued ?? "",
    dateExpired: s.dateExpired ?? "",
  };
}

function sanitizeVisa(
  v: VisaItem & { _id?: string; uploadStatus?: string },
): VisaItem {
  return {
    country: v.country ?? "",
    visaType: v.visaType ?? "",
    number: v.number ?? "",
    placeIssued: v.placeIssued ?? "",
    dateIssued: v.dateIssued ?? "",
    dateExpired: v.dateExpired ?? "",
  };
}

function sanitizeEndorse(
  e: EndorsementItem & { _id?: string; uploadStatus?: string },
): EndorsementItem {
  return {
    name: e.name ?? "",
    country: e.country ?? "",
    number: e.number ?? "",
    placeIssued: e.placeIssued ?? "",
    dateIssued: e.dateIssued ?? "",
    dateExpired: e.dateExpired ?? "",
  };
}

function sanitizeCert(
  c: CertificateItem & { _id?: string; uploadStatus?: string },
): CertificateItem {
  return {
    name: c.name ?? "",
...
function sanitizeSeaExp(
  s: SeaExperienceItem & { _id?: string },
): SeaExperienceItem {
  return {
    ...
    engineKW: String(s.engineKW ?? ""),
    company: s.company ?? "",
    rank: s.rank ?? "",
    periodFrom: s.periodFrom ?? "",
    periodTo: s.periodTo ?? "",
    areaOfOperation: s.areaOfOperation ?? "",
...
export default function CrewApplicationForm({
  ...
  useEffect(() => {
    ...
    if (initialData.passports?.length)
      passports.reset(initialData.passports.map(sanitizePassport));
    if (initialData.seamansBooks?.length)
      seamans.reset(initialData.seamansBooks.map(sanitizeSeaman));
    if (initialData.visas?.length)
      visas.reset(initialData.visas.map(sanitizeVisa));
    if (initialData.endorsements?.length)
      endorsements.reset(initialData.endorsements.map(sanitizeEndorse));
    if (initialData.stcwCertificates?.length)
      stcw.reset(initialData.stcwCertificates.map(sanitizeCert));
    if (initialData.otherCertificates?.length)
      otherCerts.reset(initialData.otherCertificates.map(sanitizeCert));
    if (initialData.seaExperience?.length)
      seaExp.reset(initialData.seaExperience.map(sanitizeSeaExp));

    // Extra docs — populate with existing names and file info (already uploaded)
    if (initialData.extraDocs?.length) {
      extraDocs.reset(
        initialData.extraDocs.map((d) => ({
          name: d.name ?? "",
          file: null,
          // Store existing file info for reference in submission
          _fileUrl: d.fileUrl,
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      ...
      coc.items.forEach((item, idx) => {
        validateJoi(
          applicationSchema.extract("licences").$_terms.items[0],
          item,
          `coc.${idx}`,
        );
      });
      ...
      if (coc.items.length === 0) {
        isValid = false;
        errors["coe"] = "At least one CoE is required";
      }
      coe.items.forEach((item, idx) => {
        // NO if-guard — always validate every item
        validateJoi(
          applicationSchema.extract("licences").$_terms.items[0],
          item,
          `coe.${idx}`,
      ...
      passports.items.forEach((item, idx) => {
        validateJoi(
          applicationSchema.extract("passports").$_terms.items[0],
          item,
          `passports.${idx}`,
        );
      });
      ...
      seamans.items.forEach((item, idx) => {
        validateJoi(
          applicationSchema.extract("seamansBooks").$_terms.items[0],
          item,
          `seamansBooks.${idx}`,
        );
      });
      ...
      visas.items.forEach((item, idx) => {
        if (item.country || item.visaType || item.number || item.dateIssued) {
          validateJoi(
            applicationSchema.extract("visas").$_terms.items[0],
            item,
            `visas.${idx}`,
          );
        }
      ...
      if (seaExp.items.length === 0) {
        isValid = false;
        errors["seaExperience"] =
          "At least one Sea Experience entry is required";
      }
  ...
  if (isView && initialData) {
    ...
    // Thin divider
    const Hr = () => (
      <hr className="border-gray-100 dark:border-white/5 my-5" />
    );
    ...
    return (
      <div className="space-y-0 divide-y divide-gray-100 dark:divide-white/5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 overflow-hidden pb-2">
            ...
            {cocItems.map((l, i) => (
              <div key={i}>
                ...
                <G3>
                  ...
                  <F
                    label="Date Issued"
                    value={formatDate(l.dateIssued as string)}
                  />
                  <F
                    label="Date Expired"
                    value={formatDate(l.dateExpired as string) || "Unlimited"}
                  />
                </G3>
              </div>
            ))}
            ...
            {d.passports!.map((p, i) => (
              <div key={i}>
                ...
                <G3>
                  ...
                  <F
                    label="Date Issued"
                    value={formatDate(p.dateIssued as string)}
                  />
                  <F
                    label="Date Expired"
                    value={formatDate(p.dateExpired as string)}
                  />
                </G3>
              </div>
            ))}
            ...
            {d.seamansBooks!.map((s, i) => (
              <div key={i}>
                ...
                <G3>
                  ...
                  <F
                    label="Date Issued"
                    value={formatDate(s.dateIssued as string)}
                  />
                  <F
                    label="Date Expired"
                    value={formatDate(s.dateExpired as string) || "Unlimited"}
                  />
                </G3>
              </div>
            ))}
            ...
            {d.visas!.map((v, i) => (
              <div key={i}>
                ...
                <G3>
                  ...
                  <F
                    label="Date Issued"
                    value={formatDate(v.dateIssued as string)}
                  />
                  <F
                    label="Date Expired"
                    value={formatDate(v.dateExpired as string)}
                  />
                </G3>
              </div>
            ))}
              ...
              <div key={i}>
                ...
                <div className="space-y-4">
                  ...
                  <G3>
                    ...
                    <F
                      label="Date Issued"
                      value={formatDate(e.dateIssued as string)}
                    />
                    <F
                      label="Date Expired"
                      value={formatDate(e.dateExpired as string) || "Unlimited"}
                    />
                  </G3>
                </div>
              </div>
              ...
              <div key={i}>
                ...
                <div className="space-y-4">
                  ...
                  <G3>
                    ...
                    <F
                      label="Date Issued"
                      value={formatDate(c.dateIssued as string)}
                    />
                    <F
                      label="Expiry Date"
                      value={formatDate(c.dateExpired as string) || "No Expiry"}
                    />
                  </G3>
                </div>
              </div>
        ...
        {(d.otherCertificates?.length ?? 0) > 0 && (
          <div className="p-5">
            <Sec n="12b" title="Other Certificates" />
            {d.otherCertificates!.map((c, i) => (
              <div key={i}>
                {i > 0 && <Hr />}
                {d.otherCertificates!.length > 1 && (
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-3">
                    Record {i + 1}
                  </p>
                )}
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                  {d.seaExperience!.map((s, i) => (
                    <tr key={i} className="align-top">
                      <td className="py-3 pr-5 font-medium text-gray-800 dark:text-white/90 whitespace-nowrap">
                        {s.vesselName || "—"}
                      </td>
                      <td className="py-3 pr-5 text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        {s.flag || "—"}
                      </td>
                      <td className="py-3 pr-5 text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        {s.vesselType || "—"}
                      ...
                      <td className="py-3 pr-5 font-medium text-brand-600 dark:text-brand-400 whitespace-nowrap">
                        {s.rank || "—"}
                      </td>
                      <td className="py-3 pr-5 text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        {formatDate(s.periodFrom as string) || "—"}
                      </td>
                      <td className="py-3 pr-5 whitespace-nowrap">
                        {s.periodTo ? (
                          <span className="text-gray-600 dark:text-gray-400">
                            {formatDate(s.periodTo as string)}
                          </span>
                        ) : (
                          <span className="inline-flex items-center rounded-full bg-green-50 dark:bg-green-500/10 px-2 py-0.5 text-[10px] font-semibold text-green-700 dark:text-green-400">
                            Present
                          </span>
                        )}
                      </td>
          ...
          <div className="space-y-3">
            {d.resume?.fileUrl && (
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 w-24 shrink-0">
                  Resume / CV
                </span>
                <a
      ...
      <div className="space-y-8">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <FormSection>
              <FormGrid cols={2}>
                ...
                <SimpleDatePicker
                  ...
                  value={scalar.dateOfAvailability}
                  onChange={(iso) => setField("dateOfAvailability", iso)}
            ...
            <FormSection>
              <FormGrid cols={3}>
                <Input
                  ...
                  value={scalar.firstName}
                  onChange={txt("firstName")}
                ...
                <Input
                  label="Last Name *"
                  placeholder="e.g. Doe"
                  value={scalar.lastName}
                  onChange={txt("lastName")}
                ...
                <Input
                  ...
                  value={scalar.nationality}
                  onChange={txt("nationality")}
                  hint={validationErrors.nationality}
                ...
                <SimpleDatePicker
                  ...
                  label="Date of Birth *"
                  placeholder="DD/MM/YYYY"
                  value={scalar.dateOfBirth}
                  onChange={(iso) => setField("dateOfBirth", iso)}
                  hint={validationErrors.dateOfBirth}
              ...
              <div className="mt-5">
                <FormGrid cols={2}>
                  <Input
                    label="Father's Name"
                    value={scalar.fatherName}
                    onChange={txt("fatherName")}
                  />
                  <Input
                    label="Mother's Name"
                    value={scalar.motherName}
                    onChange={txt("motherName")}
                  />
                </FormGrid>
              </div>
            </FormSection>
              ...
              <div className="space-y-5">
                ...
                <FormGrid cols={2}>
                  ...
                  <Input
                    ...
                    value={scalar.cellPhone}
                    onChange={txt("cellPhone")}
                    error={!!validationErrors.cellPhone}
                    hint={validationErrors.cellPhone}
                  />
                  <Input
                    label="Home Phone"
                    type="number"
                    placeholder="91112345678"
                    value={scalar.homePhone}
                    onChange={txt("homePhone")}
                  />
                  <Input
            ...
            <FormSection>
              <FormGrid cols={3}>
                <Input
                  ...
                  value={scalar.nearestAirport}
                  onChange={txt("nearestAirport")}
                  placeholder="Dehradun, India"
                />
                <Input
                  label="Km from Airport"
                  type="number"
                  value={scalar.kmFromAirport}
                  onChange={txt("kmFromAirport")}
                  placeholder="220"
                />
                <Input
                  label="Weight (kg) *"
                  placeholder="e.g. 75"
                  type="number"
                  value={scalar.weightKg}
                  onChange={txt("weightKg")}
                  error={!!validationErrors.weightKg}
                  hint={validationErrors.weightKg}
                />
                <Input
                  label="Height (cm) *"
                  placeholder="e.g. 175"
                  type="number"
                  value={scalar.heightCm}
                  onChange={txt("heightCm")}
                  error={!!validationErrors.heightCm}
                  hint={validationErrors.heightCm}
                />
                <Input
                  label="Coverall Size *"
                  placeholder="e.g. XL"
                  ...
                  error={!!validationErrors.coverallSize}
                  hint={validationErrors.coverallSize}
                />
                <Input
                  label="Safety Shoe Size *"
                  type="number"
                  placeholder="e.g. 42"
                  value={scalar.shoeSize}
                  onChange={txt("shoeSize")}
                  error={!!validationErrors.shoeSize}
                  hint={validationErrors.shoeSize}
                />
                <Input
                  label="Hair Color *"
                  placeholder="e.g. Black"
                  value={scalar.hairColor}
                  onChange={txt("hairColor")}
                  error={!!validationErrors.hairColor}
                  hint={validationErrors.hairColor}
                />
                <Input
                  label="Eye Color *"
                  placeholder="e.g. Brown"
                  value={scalar.eyeColor}
                  onChange={txt("eyeColor")}
                  error={!!validationErrors.eyeColor}
                  hint={validationErrors.eyeColor}
                />
              </FormGrid>
            </FormSection>
            ...
            <FormSection>
              <FormGrid cols={2}>
                <SimpleDatePicker
                  id="med_issued"
                  label="Medical Cert. Date Issued *"
                  value={scalar.medicalCertIssuedDate}
                  onChange={(iso) => setField("medicalCertIssuedDate", iso)}
                  error={!!validationErrors.medicalCertIssuedDate}
                  hint={validationErrors.medicalCertIssuedDate}
                />
                <SimpleDatePicker
                  id="med_exp"
                  label="Medical Cert. Expiration Date *"
                  value={scalar.medicalCertExpiredDate}
                  onChange={(iso) => setField("medicalCertExpiredDate", iso)}
                  error={!!validationErrors.medicalCertExpiredDate}
                  hint={validationErrors.medicalCertExpiredDate}
                />
              </FormGrid>
              <div className="mt-5">
                <FormGrid cols={2}>
                  ...
                  <div className="sm:col-span-2 col-span-1">
                    ...
                    <TextArea
                      ...
                      onChange={(e) =>
                        setField("nextOfKinAddress", e.target.value)
                      }
        ...
        {currentStep === 2 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              >
                <FormGrid cols={2}>
                  <Input
                    ...
                    value={item.country}
                    onChange={(e) => coc.update(idx, "country", e.target.value)}
                    error={!!validationErrors[`coc.${idx}.country`]}
                    hint={validationErrors[`coc.${idx}.country`]}
                  />
                  <Input
                    label="Grade of Licence *"
                    placeholder="e.g. Chief Mate Reg. II/2 (Unlimited)"
                    value={item.grade}
                    onChange={(e) => coc.update(idx, "grade", e.target.value)}
                    error={!!validationErrors[`coc.${idx}.grade`]}
                    hint={validationErrors[`coc.${idx}.grade`]}
                  />
                  <Input
                    label="Licence Number *"
                    placeholder="e.g. K8037816"
                    value={item.number}
                    onChange={(e) => coc.update(idx, "number", e.target.value)}
                    error={!!validationErrors[`coc.${idx}.number`]}
                    hint={validationErrors[`coc.${idx}.number`]}
                  />
                  <Input
                    label="Place of Issue"
                    placeholder="City/Country"
                    value={item.placeIssued}
                    onChange={(e) =>
                      coc.update(idx, "placeIssued", e.target.value)
                    }
                  />
                  <SimpleDatePicker
                    id={`coc_issued_${idx}`}
                    ...
                    onChange={(iso) => coc.update(idx, "dateIssued", iso)}
                    error={!!validationErrors[`coc.${idx}.dateIssued`]}
                    hint={validationErrors[`coc.${idx}.dateIssued`]}
                  />
                  <SimpleDatePicker
                    id={`coc_expiry_${idx}`}
                    label="Date Expired"
            ...
            <div className="flex justify-end pt-2">
              <AddItemButton onClick={() => coc.add(emptyLicence())}>
                Add CoC
              </AddItemButton>
            </div>
          </div>
        ...
        {currentStep === 3 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              >
                <FormGrid cols={2}>
                  <Input
                    ...
                    value={item.country}
                    onChange={(e) => coe.update(idx, "country", e.target.value)}
                    error={!!validationErrors[`coe.${idx}.country`]}
                    hint={validationErrors[`coe.${idx}.country`]}
                  />
                  <Input
                    label="Grade of Licence *"
                    placeholder="e.g. Second Officer II/1"
                    value={item.grade}
                    onChange={(e) => coe.update(idx, "grade", e.target.value)}
                    error={!!validationErrors[`coe.${idx}.grade`]}
                    hint={validationErrors[`coe.${idx}.grade`]}
                  />
                  <Input
                    label="Licence Number *"
                    placeholder="Number"
                    value={item.number}
                    onChange={(e) => coe.update(idx, "number", e.target.value)}
                    error={!!validationErrors[`coe.${idx}.number`]}
                    hint={validationErrors[`coe.${idx}.number`]}
                  />
                  <Input
                    label="Place of Issue"
                    placeholder="City/Country"
                    value={item.placeIssued}
                    onChange={(e) =>
                      coe.update(idx, "placeIssued", e.target.value)
                    }
                  />
                  <SimpleDatePicker
                    id={`coe_issued_${idx}`}
                    label="Date Issued *"
                    value={item.dateIssued}
                    onChange={(iso) => coe.update(idx, "dateIssued", iso)}
                    error={!!validationErrors[`coe.${idx}.dateIssued`]}
                    hint={validationErrors[`coe.${idx}.dateIssued`]}
                  />
                  <SimpleDatePicker
                    id={`coe_expiry_${idx}`}
                    label="Date Expired"
            ...
            <div className="flex justify-end pt-2">
              <AddItemButton onClick={() => coe.add(emptyLicence())}>
                Add CoE
              </AddItemButton>
            </div>
          </div>
        ...
        {currentStep === 4 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              >
                <FormGrid cols={1}>
                  <Input
                    ...
                    placeholder="e.g. Z6824864"
                    value={item.number}
                    onChange={(e) =>
                      passports.update(idx, "number", e.target.value)
                    }
                    error={!!validationErrors[`passports.${idx}.number`]}
                    hint={validationErrors[`passports.${idx}.number`]}
                  />
                  <Input
                    label="Country *"
                    ...
                    value={item.country}
                    onChange={(e) =>
                      passports.update(idx, "country", e.target.value)
                    }
                    error={!!validationErrors[`passports.${idx}.country`]}
                    hint={validationErrors[`passports.${idx}.country`]}
                  />
                  <Input
                    label="Place of Issue"
                    ...
                    value={item.placeIssued}
                    onChange={(e) =>
                      passports.update(idx, "placeIssued", e.target.value)
                    }
                  />
                  <SimpleDatePicker
                    id={`pass_issued_${idx}`}
                    label="Date Issued *"
                    value={item.dateIssued}
                    onChange={(iso) => passports.update(idx, "dateIssued", iso)}
                    error={!!validationErrors[`passports.${idx}.dateIssued`]}
                    hint={validationErrors[`passports.${idx}.dateIssued`]}
                  />
                  <SimpleDatePicker
                    id={`pass_expiry_${idx}`}
                    label="Date Expired"
            ...
            <div className="flex justify-end pt-2">
              <AddItemButton onClick={() => passports.add(emptyPassport())}>
                Add Passport
              </AddItemButton>
            </div>
          </div>
        ...
        {currentStep === 5 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              >
                <FormGrid cols={2}>
                  <Input
                    ...
                    value={item.country}
                    onChange={(e) =>
                      seamans.update(idx, "country", e.target.value)
                    }
                    error={!!validationErrors[`seamansBooks.${idx}.country`]}
                    hint={validationErrors[`seamansBooks.${idx}.country`]}
                  />
                  <Input
                    label="Number *"
                    placeholder="e.g. MUM132428"
                    value={item.number}
                    onChange={(e) =>
                      seamans.update(idx, "number", e.target.value)
                    }
                    error={!!validationErrors[`seamansBooks.${idx}.number`]}
                    hint={validationErrors[`seamansBooks.${idx}.number`]}
                  />
                  <Input
                    label="Place of Issue"
                    ...
                    value={item.placeIssued}
                    onChange={(e) =>
                      seamans.update(idx, "placeIssued", e.target.value)
                    }
                  />
                  <SimpleDatePicker
                    id={`sb_issued_${idx}`}
                    label="Date Issued *"
                    value={item.dateIssued}
                    onChange={(iso) => seamans.update(idx, "dateIssued", iso)}
                    error={!!validationErrors[`seamansBooks.${idx}.dateIssued`]}
                    hint={validationErrors[`seamansBooks.${idx}.dateIssued`]}
                  />
                  <SimpleDatePicker
                    id={`sb_expiry_${idx}`}
                    label="Date Expired"
            ...
            <div className="flex justify-end pt-2">
              <AddItemButton onClick={() => seamans.add(emptySeaman())}>
                Add Seaman&apos;s Book
              </AddItemButton>
            </div>
          </div>
        ...
        {currentStep === 6 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              >
                <FormGrid cols={2}>
                  <Input
                    ...
                    value={item.country}
                    onChange={(e) =>
                      visas.update(idx, "country", e.target.value)
                    }
                    error={!!validationErrors[`visas.${idx}.country`]}
                    hint={validationErrors[`visas.${idx}.country`]}
                  />
                  <Input
                    label="Visa Type *"
                    placeholder="e.g. C1/D, Schengen"
                    value={item.visaType}
                    onChange={(e) =>
                      visas.update(idx, "visaType", e.target.value)
                    }
                    error={!!validationErrors[`visas.${idx}.visaType`]}
                    hint={validationErrors[`visas.${idx}.visaType`]}
                  />
                  <Input
                    label="Visa Number *"
                    placeholder="e.g. N2791258"
                    value={item.number}
                    onChange={(e) =>
                      visas.update(idx, "number", e.target.value)
                    }
                    error={!!validationErrors[`visas.${idx}.number`]}
                    hint={validationErrors[`visas.${idx}.number`]}
                  />
                  <Input
                    label="Place of Issue"
                    ...
                    value={item.placeIssued}
                    onChange={(e) =>
                      visas.update(idx, "placeIssued", e.target.value)
                    }
                  />
                  <SimpleDatePicker
                    id={`visa_issued_${idx}`}
                    label="Date Issued *"
                    value={item.dateIssued}
                    onChange={(iso) => visas.update(idx, "dateIssued", iso)}
                    error={!!validationErrors[`visas.${idx}.dateIssued`]}
                    hint={validationErrors[`visas.${idx}.dateIssued`]}
                  />
                  <SimpleDatePicker
                    id={`visa_expiry_${idx}`}
                    label="Date Expired"
            ...
            <div className="flex justify-end pt-2">
              <AddItemButton onClick={() => visas.add(emptyVisa())}>
                Add Visa
              </AddItemButton>
            </div>
          </div>
        ...
        {currentStep === 7 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <FormGrid cols={2}>
                  <div className="sm:col-span-2">
                    <Input
                      ...
                      value={item.name}
                      onChange={(e) =>
                        endorsements.update(idx, "name", e.target.value)
                      }
                      error={!!validationErrors[`endorsements.${idx}.name`]}
                      hint={validationErrors[`endorsements.${idx}.name`]}
                    />
                  </div>
                  <Input
                    label="Number"
                    ...
                    value={item.number}
                    onChange={(e) =>
                      endorsements.update(idx, "number", e.target.value)
                    }
                  />
                  <Input
                    label="Place of Issue"
                    value={item.placeIssued}
                    onChange={(e) =>
                      endorsements.update(idx, "placeIssued", e.target.value)
                    }
                  />
                  <SimpleDatePicker
                    id={`endors_issued_${idx}`}
                    label="Date Issued"
                    value={item.dateIssued}
                    onChange={(iso) =>
                      endorsements.update(idx, "dateIssued", iso)
                    }
                  />
                  <SimpleDatePicker
            ...
            <div className="flex justify-end pt-2">
              <AddItemButton onClick={() => endorsements.add(emptyEndorse())}>
                Add Endorsement
              </AddItemButton>
            </div>
          </div>
                    ...
                    <FormGrid cols={2}>
                      <div className="sm:col-span-2">
                        <Input
                          ...
                          value={item.name}
                          onChange={(e) =>
                            stcw.update(idx, "name", e.target.value)
                          }
                          error={!!validationErrors[`stcw.${idx}.name`]}
                          hint={validationErrors[`stcw.${idx}.name`]}
                        />
                      </div>
                      <Input
                        label="Course Number"
                        placeholder="e.g. AFF/0221/AD/0651"
                        value={item.courseNumber}
                        onChange={(e) =>
                          stcw.update(idx, "courseNumber", e.target.value)
                        }
                      />
                      <Input
                        label="Place of Issue"
                        ...
                        value={item.placeIssued}
                        onChange={(e) =>
                          stcw.update(idx, "placeIssued", e.target.value)
                        }
                      />
                      <SimpleDatePicker
                        id={`stcw_issued_${idx}`}
                        label="Date Issued"
                        value={item.dateIssued}
                        onChange={(iso) => stcw.update(idx, "dateIssued", iso)}
                      />
                      <SimpleDatePicker
                        id={`stcw_expiry_${idx}`}
                        label="Expiration Date"
                        value={item.dateExpired}
                        onChange={(iso) => stcw.update(idx, "dateExpired", iso)}
                      />
                    </FormGrid>
                  </RepeatCard>
                ))}
                <div className="flex justify-end pt-2">
                  <AddItemButton onClick={() => stcw.add(emptyCert())}>
        ...
        {currentStep === 9 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="space-y-4">
                ...
                >
                  <div className="space-y-5">
                    <FormGrid cols={3}>
                      <Input
                        label="Vessel Name *"
                        placeholder="e.g. LPG GAS TEESA"
                        value={item.vesselName}
                        onChange={(e) =>
                          seaExp.update(idx, "vesselName", e.target.value)
                        }
                        error={
                          !!validationErrors[`seaExperience.${idx}.vesselName`]
                        }
                        hint={
                          validationErrors[`seaExperience.${idx}.vesselName`]
                        }
                      ...
                      <Input
                        ...
                        value={item.flag}
                        onChange={(e) =>
                          seaExp.update(idx, "flag", e.target.value)
                        }
                        error={!!validationErrors[`seaExperience.${idx}.flag`]}
                        hint={validationErrors[`seaExperience.${idx}.flag`]}
                      />
                      <Select
                        label="Vessel Type *"
                        ...
                        onChange={(v) => seaExp.update(idx, "vesselType", v)}
                        error={
                          !!validationErrors[`seaExperience.${idx}.vesselType`]
                        }
                        hint={
                          validationErrors[`seaExperience.${idx}.vesselType`]
                        }
                    ...
                    <FormGrid cols={3}>
                      ...
                      <Input
                        ...
                        value={item.engineType}
                        onChange={(e) =>
                          seaExp.update(idx, "engineType", e.target.value)
                        }
                        error={
                          !!validationErrors[`seaExperience.${idx}.engineType`]
                        }
                        hint={
                          validationErrors[`seaExperience.${idx}.engineType`]
                        }
                    ...
                    <FormGrid cols={2}>
                      <Input
                        ...
                        placeholder="e.g. UNI Fleet SDN BHD"
                        value={item.company}
                        onChange={(e) =>
                          seaExp.update(idx, "company", e.target.value)
                        }
                        error={
                          !!validationErrors[`seaExperience.${idx}.company`]
                        }
                        hint={validationErrors[`seaExperience.${idx}.company`]}
                      />
                      <Input
                        label="Rank *"
                        ...
                        value={item.rank}
                        onChange={(e) =>
                          seaExp.update(idx, "rank", e.target.value)
                        }
                        error={!!validationErrors[`seaExperience.${idx}.rank`]}
                        hint={validationErrors[`seaExperience.${idx}.rank`]}
                      />
                      <SimpleDatePicker
                        id={`sea_from_${idx}`}
                        label="Period From *"
                        ...
                        onChange={(iso) =>
                          seaExp.update(idx, "periodFrom", iso)
                        }
                        error={
                          !!validationErrors[`seaExperience.${idx}.periodFrom`]
                        }
                        hint={
                          validationErrors[`seaExperience.${idx}.periodFrom`]
                        }
                      ...
                      <SimpleDatePicker
                        ...
                        label="Period To"
                        value={item.periodTo}
                        onChange={(iso) => seaExp.update(idx, "periodTo", iso)}
                        error={
                          !!validationErrors[`seaExperience.${idx}.periodTo`]
                        }
                        hint={validationErrors[`seaExperience.${idx}.periodTo`]}
                      />
                    </FormGrid>
                    <div className="border-t border-gray-100 pt-4 dark:border-gray-800">
                      ...
                      <TextArea
                        ...
                        onChange={(e) =>
                          seaExp.update(idx, "jobDescription", e.target.value)
                        }
                        error={
                          !!validationErrors[
              ...
              <div className="flex justify-end pt-2">
                <AddItemButton onClick={() => seaExp.add(emptySeaExp())}>
                  Add Vessel
                </AddItemButton>
              </div>
            </div>
              ...
              <FormGrid cols={2}>
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Image format preferred (JPG/PNG)
                  </p>
                </div>
            ...
            <FormSection>
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-500">
                  Add any other relevant certificates, appraisals, or medical
                  records.
                </p>
              </div>
