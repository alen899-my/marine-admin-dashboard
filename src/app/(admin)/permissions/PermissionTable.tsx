...
export default function PermissionTable({
  ...
  const columns = [
    ...
    },
    {
      header: "Resource",
      render: (
        p: any, // Changed type to any to handle populated object
      ) => (
  ...
  const fetchPermissions = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/permissions/all-permission?${params.toString()}`,
        {
      ...
      if (result.resources && Array.isArray(result.resources)) {
        ...
        setResourceOptions((prev) => {
          if (JSON.stringify(prev) === JSON.stringify(formattedResources))
            return prev;
  ...
  const handleEdit = (p: Permission) => {
    ...
    const resId =
      typeof p.resourceId === "object" ? p.resourceId._id : p.resourceId;
    setEditData({
      slug: p.slug,
      name: p.name || "",
  ...
  const handleUpdate = async (force = false) => {
    ...
    try {
      ...
      setPermissions((prev) =>
        prev.map((p) => (p._id === selectedPermission._id ? result.data : p)),
      );
      ...
      >
        <div className="text-[13px] py-1 max-w-md mx-auto">
            ...
            <section className="space-y-1.5">
              ...
              <div className="flex justify-between gap-4">
                ...
                <span className="font-medium text-right capitalize  tracking-tight text-gray-700 dark:text-gray-300">
                  {(selectedPermission?.resourceId as ResourceRef)?.name ||
          ...
          <div className="mt-8 pt-4 border-t border-gray-200 dark:border-white/10 flex items-center justify-between">
            ...
            <Badge
              color={
                selectedPermission?.status === "active" ? "success" : "default"
              }
            >
