...
const formatDateTimeLocal = (dateStr: string | Date | undefined) => {
  ...
  // Adjust to local time and slice to 16 chars: YYYY-MM-DDTHH:mm
...
export default function VoyageAnalysisPage() {
  ...
  // Array for Table UI (Now Functional)
  ...
  const [isPerformanceLoading, setIsPerformanceLoading] = useState(false); // New Loading State
  ...
  useEffect(() => {
    const fetchVessels = async () => {
      ...
      try {
        // Using optimized lean query via all=true
        const response = await fetch("/api/vessels?all=true");
  ...
  // --- Fetch Voyages when a Vessel is selected ---
  useEffect(() => {
    if (!vessel) {
      setVoyagesList([]);
      return;
    ...
    const fetchVoyages = async () => {
      ...
      try {
        // Fetch voyages filtered by the selected vessel (using optimized dropdown mode)
        const response = await fetch(
          `/api/voyages?vesselId=${vessel}&status=active`,
        );
        const result = await response.json();
        ...
        if (Array.isArray(result)) {
          ...
          // Fallback if the API returns a paginated object instead of a flat array
          setVoyagesList(
            result.data.map((voy: any) => ({
              value: voy._id,
              label: voy.voyageNo,
            })),
          );
  ...
  // --- Data Fetching Logic (Effect) ---
  useEffect(() => {
    if (!legId) return;

    // This is where you call your API to get existing values
    const fetchVoyagePerformance = async () => {
      setIsPerformanceLoading(true); // Start Loading
      try {
        // Updated path to match: src\app\api\voyage-performance\[id]\route.ts
        const response = await fetch(`/api/voyage-performance/${legId}`);
        const data = await response.json();

        if (data) {
          // Mapping DB values to State
          setDepDt(formatDateTimeLocal(data.operational?.departure?.eventTime));
          setArrDt(formatDateTimeLocal(data.operational?.arrival?.eventTime));
          setPlannedNm(data.operational?.departure?.plannedNm || "");
          setRfaDt(formatDateTimeLocal(data.operational?.departure?.rfaDt));
          setDepVlsfo(data.operational?.departure?.stats?.robVlsfo || "");
          setArrVlsfo(data.operational?.arrival?.stats?.robVlsfo || "");
          setDepLsmgo(data.operational?.departure?.stats?.robLsmgo || "");
          setArrLsmgo(data.operational?.arrival?.stats?.robLsmgo || "");
          ...
          if (data.dailyReports) {
            setNoonReports(
              data.dailyReports.map((r: any) => ({
                ...
                noonHrs: 24, // Assuming standard noon-to-noon
        ...
        setIsPerformanceLoading(false); // End Loading
      }
    };

    fetchVoyagePerformance();
  }, [legId]);

  // --- Performance Calculation Engine (useMemo) ---
  const performance = useMemo(() => {
    // 1. Leg Time Logic
    let days = 0;
    let durationStr = "0d 0h 0m"; // Initial default value
    ...
    if (depDt && arrDt) {
      ...
      if (diff > 0) {
        ...
        // Calculate breakdown for 3d 8h 45m format
    ...
    // Validation: Flag if ROB increases (Arrival > Departure)
    ...
    return {
      ...
      durationStr, // New formatted string for the UI
  ...
  // 1. Helper to get the label of selected items
  const vesselName =
    vesselsList.find((v) => v.value === vessel)?.label || "N/A";
  const voyageNo = voyagesList.find((v) => v.value === legId)?.label || "N/A";

  // 2. Format the data object specifically for the PDF table
  const pdfData = {
    vessel: vesselName,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2.5">
              ...
              <div className="space-y-1">
                ...
                <SearchableSelect
                  ...
                  disabled={!vessel || isLoadingVoyages}
                />
              </div>
...
function KPIRow({
  ...
  return (
    <div className="flex gap-3 group">
      <div className="w-11 h-11 rounded-xl bg-gray-50 dark:bg-gray-800 flex items-center justify-center shrink-0 border border-transparent group-hover:border-gray-200 dark:group-hover:border-gray-700 transition-all shadow-sm">
        {icon}
      </div>
      <div className="space-y-0">
        ...
        {hint && (
          <p className="text-[10px] text-gray-300 dark:text-gray-600 font-mono mt-0.5 italic">
            {hint}
          </p>
        )}
      </div>
