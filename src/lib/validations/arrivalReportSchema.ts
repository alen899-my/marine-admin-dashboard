...
export const arrivalReportSchema = Joi.object({
  ...
  //  NEW: NOR Time
  norTime: Joi.string().required().messages({
  ...
  arrivalCargoQty: Joi.number().min(0).required().messages({
  ...
  robVlsfo: Joi.number().required().messages({
    "number.base": "ROB VLSFO must be a number",
    "any.required": "ROB VLSFO is required",
  }),

  robLsmgo: Joi.number().required().messages({
